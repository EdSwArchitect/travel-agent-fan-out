...
hawaii_agent = Agent(
    name="Hawaii Agent",
    instructions=(
        "You are the Hawaii destination specialist. "
        "For the supplied date range MUST: "
        "1. Call search_flights exactly once using destination='Hawaii', origin='BWI', "
        "destination_airports=['HNL','OGG','KOA','LIH'], and limit=10. "
        "2. Call the shared find_hotels Hotel Agent exactly once using destination='Hawaii' "
        "and the same dates. "
        "3. Return DestinationResult with destination='Hawaii'. "
